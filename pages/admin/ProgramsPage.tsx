                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                <th className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Program Name</th>
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {programs.map(prog => (
                                <tr key={prog.id}>
                                    <td className="px-4 py-3 whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">{prog.name}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-gray-500 dark:text-gray-400">{prog.departmentName}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-gray-500 dark:text-gray-400">{prog.courseCount}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-gray-500 dark:text-gray-400">{prog.duration}</td>
                                    <td className="px-4 py-3 whitespace-nowrap space-x-4">
                                        <button onClick={() => handleOpenModal(prog)} className="text-secondary dark:text-blue-400 hover:text-secondary-dark font-medium">Edit</button>
                                        <button className="text-secondary dark:text-blue-400 hover:text-secondary-dark font-medium">Manage Courses</button>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={selectedProgram ? 'Edit Program' : 'Add Program'}>
                <form className="space-y-4" onSubmit={e => e.preventDefault()}>
                    <div>
                        ...
                        <input type="text" id="prog-name" value={formData.name} onChange={handleFormChange} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                     <div>
                        <label htmlFor="prog-departmentId" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Department</label>
                        <select id="prog-departmentId" value={formData.departmentId} onChange={handleFormChange} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary">
                            {departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                        </select>
                    </div>
                     <div>
                        <label htmlFor="prog-duration" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Duration</label>
                        <input type="text" id="prog-duration" value={formData.duration} onChange={handleFormChange} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
