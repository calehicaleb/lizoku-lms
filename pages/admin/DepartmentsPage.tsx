                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                <th className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Department Name</th>
                                <th className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Head of Department</th>
                                <th className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Programs</th>
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {departments.map(dept => (
                                <tr key={dept.id}>
                                    <td className="px-4 py-3 whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">{dept.name}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-gray-500 dark:text-gray-400">{dept.head}</td>
                                    <td className="px-4 py-3 whitespace-nowrap text-gray-500 dark:text-gray-400">{dept.programCount}</td>
                                    <td className="px-4 py-3 whitespace-nowrap space-x-4">
                                        <button onClick={() => handleOpenModal(dept)} className="text-secondary dark:text-blue-400 hover:text-secondary-dark font-medium">Edit</button>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={selectedDept ? 'Edit Department' : 'Add Department'}>
                <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                    <div>
                        ...
                        <input type="text" id="dept-name" value={formData.name} onChange={handleFormChange} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                     <div>
                        <label htmlFor="dept-head" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Head of Department</label>
                        <input type="text" id="dept-head" value={formData.head} onChange={handleFormChange} className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
