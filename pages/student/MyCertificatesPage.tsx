            ...
            {certificates.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {certificates.map(cert => (
                        <div key={cert.id} className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden border-4 border-primary/20">
                            <div className="p-6 bg-secondary-light/30 text-center border-b-2 border-dashed border-primary/30">
                                <Icon name="BadgeCheck" className="h-16 w-16 text-primary mx-auto mb-4" />
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Certificate of Completion</p>
                                <h2 className="text-xl font-bold text-secondary mt-2">{cert.courseName}</h2>
                            </div>
                            <div className="p-6">
                                <p className="text-sm text-gray-600">Issued to:</p>
                                <p className="font-semibold text-gray-800">{cert.studentName}</p>
                                <p className="text-sm text-gray-600 mt-3">Issued on:</p>
                                <p className="font-semibold text-gray-800">{new Date(cert.issueDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                <button
                ...
                <div className="text-center py-16 px-6 bg-white rounded-lg shadow-sm">
                    ...
                    <p className="mt-1 text-sm text-gray-500">You haven't earned any certificates. Complete a course to receive one!</p>
                </div>
            )}
        </div>
    );
